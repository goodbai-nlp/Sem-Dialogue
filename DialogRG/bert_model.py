...
class SentTransformer(nn.Module):
    ...
    def forward(self, batch):
        # encoding
        src_mask_inv = batch["src_mask"] == 0   # [batch, seq]
        batch["src_mask_inv"] = src_mask_inv
        src_emb = self.embedding(batch["src"])  # [batch, seq, dim]
...
class BERTEncoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        # print("src_input", batch["src"].size())
        output = self.encoder(input_ids=batch["src"], attention_mask=batch["src_mask"], output_hidden_states=True)
...
class GraphTransformer(nn.Module):
    def __init__(self, config, bpemb, vocab, relation_vocab):
        ...
        assert config.d_relation * config.g_n_head == config.d_concept

        self.position_encoder = PositionalEncoding(config.d_concept)

        # encoder_layer = nn.TransformerEncoderLayer(
        #     config.d_model, config.n_head, dim_feedforward=1024, dropout=config.dropout
        # )
        encoder_layer = TransformerEncoderLayer(
            d_model=config.d_concept,
        ...
        encoder_norm = nn.LayerNorm(config.d_concept)
    ...
    def forward(self, batch):
        ...
        structure_emb = self.relation_embedding(batch["rel"])  # [batch, seq, seq, s_dim]
...
class DualTransformer(nn.Module):
    def __init__(self, config, word_emb, con_emb, word_vocab, concept_vocab, relation_vocab):
        ...
        self.enc_word_embedding = self.build_embedding(word_emb, word_vocab, self.config.d_model)
        self.vocab_inv = {v: k for k, v in word_vocab.items()}
        ...
        if config.dual_enc and self.concept_vocab is not None and relation_vocab is not None:
            if config.share_con_vocab:
                ...
                self.enc_concept_embedding = self.build_embedding(con_emb, concept_vocab, self.config.d_concept)
        ...
        if config.dual_enc:
            ...
            decoder_layer = DoubleAttnTransformerDecoderLayer(
                d_model=config.d_model,
                d_enc=config.d_enc + config.d_concept if config.dual_enc else config.d_enc,
                heads=config.n_head,
                d_ff=1024,
                dropout=config.dropout,
                att_drop=config.dropout,
        ...
        self.projector = nn.Linear(config.d_model, self.word_vocab_size)
    ...
    def decode_into_string(self, ids, bert_token=False):
        try:
            ...
            pass
        if bert_token:
            return " ".join(bert_tokenizer.convert_ids_to_tokens(ids))
        if self.word_vocab is not None:
            return " ".join([self.vocab_inv[x] for x in ids])
    ...
    def forward(self, batch):
        ...
        if batch["tgt_ref"] is not None:
            ...
            train_right = ((preds == target_ref).float() * batch["tgt_mask"]).sum()
